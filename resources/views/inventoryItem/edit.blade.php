...
<div class="main-content row" autoscroll="true" bs-affix-target="" init-ripples="" style="margin-top:-100px">
  <section class="forms-basic">
   ...
   <!-- left content -->
	  ...
	  <div>
      <form class="form-horizontal" method="post" action="{{ url('/inventoryItem/update', array($inventoryitem->InvId)) }}">
			<fieldset>
			...
			<tr class="box-section">
				...
				<label for="InvCatId" class="control-label label-left" required><i class="fa fa-list"></i> Category</label>
                    ...
                    <?php
						$id = $inventoryitem->InvCatId;
						$invcategory = DB::table('invcategory')->where('InvCatId', '=', $id)->first();
					?>
					@if($invcategory)
					<option value="{{{$invcategory->InvCatId}}}"> {{ $invcategory->InvName }} </option>
					@endif
				...
				<div class="form-group">
					...
					<input type="text" name="InvItemName" id="InvItemName" style="border:thin #ede solid;	width:93%;	padding:5px;	border-radius:2px;margin:auto 2% auto 4%; color:#999"@if($inventoryitem) value="{{$inventoryitem->InvItemName}}"@endif Required onblur="convert_name()"> 
				</div>							
			  </td>
				<td width="50%"> 
				...
				<select class='sel-opt-right' name='VendorId' id='VendorId' required>
					<?php  
						$vid = $inventoryitem->VendorId;                         
						$vendor = DB::table('vendor')->where('VendorId', '=', $vid)->first();
					?>
					@if($vendor)
					<option value="{{{$vendor->VendorId}}}"> {{ $vendor->VendorName }} </option>
					@endif
                    <option value="">Select Vendor - Manufactural</option>				
				</select>
				</div>
								
					<div class="form-group">	  
						<label for="SerialNo" class="control-label label-right" required><i class="fa fa-book"></i> Serial Number</label>
							<input type="text" name="SerialNo" id="SerialNo" style="border:thin #ede solid;	width:93%;	padding:5px;	border-radius:2px;margin:auto 0% auto 3%; color:#999" onkeyup="convert_serial()"@if($inventoryitem) value="{{$inventoryitem->SerialNo}}"@endif Required> 	 						
				    </div>
				</td>
			</tr>
			...
			<tr class="box-section">
				...
				<div class="form-group">
					...
					<input type="number" name="Quantity" id="Quantity" style="border:thin #ede solid;	width:93%;	padding:5px;	border-radius:2px;margin:auto 2% auto 4%; color:#999"@if($inventoryitem) value="{{$inventoryitem->Quantity}}"@endif Required>
				</div>							
				</td>
				<td width="50%"> 
				<div class="form-group">
					<label for="Cost" class="control-label label-right"><i class="fa fa-usd"></i> Cost</label>
					<input type="text" name="Cost" id="Cost" style="border:thin #ede solid;	width:93%;	padding:5px;	border-radius:2px;margin:auto 0% auto 3%; color:#999" @if($inventoryitem) value="{{$inventoryitem->Cost}}"@endif Required>
				</div> 
				</td>
			</tr>
			...
			<tr class="box-section">
				<td width="50%"> 
					<div class="form-group">
                        ...
                        <div class="radio" style="margin-left:39px"> 
                            @if($inventoryitem->Active == '1') 
                            &nbsp;&nbsp;&nbsp;&nbsp;<input type="radio" name="Active" value="1" id="Active_0" checked /> Yes   <br> <br> 
                            &nbsp;&nbsp;&nbsp;&nbsp;<input type="radio" name="Active" value="0" id="Active_1" />  No
                            

                            @elseif($inventoryitem->Active == '0') 
                            &nbsp;&nbsp;&nbsp;&nbsp; <input type="radio" name="Active" value="1" id="Active_2" /> Yes br> <br> 
                            &nbsp;&nbsp;&nbsp;&nbsp; <input type="radio" name="Active" value="0" id="Active_3" checked /> No
                            @endif										  
                        </div>
                    </div>
				...
				<td width="50%"> 
					<input type="hidden" name="CreatedBy" id="CreatedBy" class="form-control" @if($inventoryitem) value="{{$inventoryitem->CreatedBy}}"@endif readonly >
