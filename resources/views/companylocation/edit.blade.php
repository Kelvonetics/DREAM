...
<div class="main-content row" autoscroll="true" bs-affix-target="" init-ripples bg="" style="margin-top:-100px">
  <section class="tables-data">
	...
	<div class="">
      ...
      <form class="form-horizontal" method="post" action="{{ url('/companylocation/update', array($companylocation->LocationId)) }}">
      <LEGEND> Edit Company Location</LEGEND>
			...
			<tr class="box-section">
				<td width="50%">
					<div class="form-group">
						...
						<input type="text" rows="2" name="LocationName" id="LocationName" placeholder="Location Name" style="border:thin #ede solid;	width:93%;	padding:5px;	border-radius:2px;margin:auto 2% auto 4%; color:#999"@if($companylocation) value="{{$companylocation->LocationName}}"@endif Required onblur="convert_name()"> 
					</div>
					
					<div class="form-group">
						<label for="State" class="control-label label-left"><i class="fa fa-map-marker"></i> State</label>
						<input type="text" rows="2" name="State" id="State" placeholder="State" style="border:thin #ede solid;	width:93%;	padding:5px;	border-radius:2px;margin:auto 2% auto 4%; color:#999"@if($companylocation) value="{{$companylocation->State}}"@endif Required onblur="convert_state()"> 
					</div>
					...
					<div class="form-group">
						...
						<select class='sel-opt-left' name='CountryId' id='CountryId' required>
							<?php                            
								$id = $companylocation->CountryId;
								$country = DB::table('country')->where('CountryId', '=', $id)->first();
							?>
							@if($company)
					...
					<input type="hidden" name="CreatedBy" id="CreatedBy" class="form-control" value="{{$companylocation->CreatedBy}}" readonly > 
                    <input type="hidden" name="_token" value="{{{ csrf_token() }}}">							
			  </td>
				<td width="50%"> 
					<div class="form-group">
						<label for="Address" class="control-label label-left"><i class="fa fa-map-marker"></i> Address</label>
						<textarea type="text" name="Address" id="Address" rows="4" style="border:thin #ede solid; width:93%;	padding:5px;	border-radius:2px;margin:auto 0% auto 3%; color:#999" Required onkeyup="convert_addr()">@if($companylocation){{$companylocation->Address}}@endif</textarea>
					</div> 
				</td>
