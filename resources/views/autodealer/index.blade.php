...
<div class="main-content row" autoscroll="true" bs-affix-target="" init-ripples bg="" style="margin-top:-100px">
  <section class="tables-data">
	...
	<h1>      <i class="fa fa-taxi"></i>     Auto Dealer </h1> 
            <p class="lead"> The auto dealer Make module  allows you to efficiently manage your entire  auto dealer. With real-time visibility into  auto dealer <br>performance and powerful analytics, it’s easier to ultimately maximize your return on assets (ROA). </p> <br>
	 <a href="{{route('autodealer.add')}}" class='btn btn-primary' style='font-size:11px;color:white' title='Create New Auto Dealer For Part/Asset'>New Dealer</a>
	...
	<div class="">
        ...
        <table id="example" class="table table-full table-full-small dealer" cellspacing="0" width="100%">
        <thead>
            <tr>
                <th> Auto Dealer </th>
                <th> Phone No </th>
		...
		@if($autodealers)
            ...
            <tr>
                <td>{{$autodealers->DealerName}}</td>
                <td>{{$autodealers->PhoneNo}}</td>
                <td>{{$autodealers->DealerEmail}}</td>
                <td>{{$autodealers->DealerAddress}}</td>

                <td class="actions pull-right">					
                <a href="{{route('autodealer.edit', $autodealers->DealerId)}}" class='btn btn-info fa fa-edit' style='color:white; font-size:9px'></a>	 
                </td>
            </tr>
            @endforeach
		@endif
