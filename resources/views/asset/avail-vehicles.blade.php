             ...
             <!-- left content -->
              <div class="col-md-12" style="margin-top:-40px"> <br>
				...
				<?php 
                    ...
                    session()->put('from', $fr_date); session()->put('to', $to_date);
				?>
				<div class="row">
					<div class='col-sm-3'>
						...
						<div class='input-group date'>
							...
							<input type='text' class="form-control" id='fromdatetime' name='fromdatetime' placeholder="Available From"  value="<?= $_GET['fromdatetime']; ?>"  /> 	
					...
					<div class='col-sm-3'>
						...
						<div class='input-group date'>
							...
							<input type='text' class="form-control" id='todatetime' name='todatetime' placeholder="Available To" value="<?= $_GET['todatetime']; ?>" /> 	
							<span class="input-group-addon"><span class="glyphicon glyphicon-calendar"></span>
							<button type="submit" class='btn btn-default fa fa-search' id="search_btn"  ></button></span>						
						</div>
						</div>
					</div>
				...
				<div id="unAssignDrvDiv">
                  <table class="table table-full table-full-small drivertab driverDTable" cellspacing="0">
                   ...
                   <tbody id="tbody">
					...
					<?php		      
					@$start_dt = strtotime($_GET['fromdatetime']);  
                    @$end_dt =  strtotime($_GET['todatetime']);  
						$today_from = date("m/d/Y 00:00"); $today_to = date("m/d/Y 23:59");	
						if($start_dt == "")  { 
                       ...
                       /*$st_query = '2017-02-28 0:00:00';
						$end_query = '2017-03-28 0:00:00';

						$sql1 ="SELECT * FROM operator JOIN assignasset ON operator.UserId = assignasset.UserId WHERE(assignasset.StartTime > '{$st_query}' AND assignasset.EndTime < '{$end_query}') ORDER BY assignasset.StartTime ASC";*/
						
						$assets = DB::table('assignasset')
						->where('StartTime', '>=', $st_query)
						->where('EndTime', '<=', $end_query)->get(); 
						?>   
						...
						<a class="waves-effect waves-light btn assignBtn btn-warning"  onmouseover="return warn();" onclick="assign(<?= $assets->DeptId.','.$assets->UserId ?>)" data-toggle="modal" title="Assign Driver" data-target="#assignAsset" href="#" style="color:#fff;font-size:10px"> + </a>							
					</td>
					</tr>
					...
					@endif	
						...
						$sql2 = "SELECT * from operator WHERE NOT EXISTS (SELECT * FROM assignasset WHERE operator.UserId = assignasset.UserId AND assignasset.StartTime >= '{$st_query}' AND assignasset.EndTime <= '{$end_query}')";
						$result2 = $conn->query($sql2);
						if (@$result2->num_rows > 0)
						{
							while($records = $result2->fetch_assoc())
							 {
								   
							echo  '<tr style=""> 	<td> <span style="color:#fff">1</span>'; ?>						
																		 
						   <img src="{{URL::asset('assets/img/pass.png')}}" class="img-responsive pull-left" height="10px" width="12px" style="margin-top:8px">
						   <?php //plateno
						   
						   echo  '</td>	<td>';

								$uid = $records['UserId'];
								$sqlfname = "SELECT * FROM users WHERE UserId = '{$uid}' ";
								$rst = $conn->query($sqlfname);
								$gname = $rst->fetch_assoc();
								  ?>
								  <img src="{{URL::asset('assets/img/pass.png')}}"
								 height="10px" width="12px" class="">
								  <?php echo ' &nbsp;
								  &nbsp; 
								 '.$gname['FirstName']." ".$gname['LastName']; ?>
								  <?php

							echo  '</td>	<td>';
									
								$did = $records['DeptId'];
								$sql3 = "SELECT * FROM department WHERE DeptId = '{$did}' ";
								$result3 = $conn->query($sql3);
								$depts = $result3->fetch_assoc();
								echo  $depts['DeptName'];
