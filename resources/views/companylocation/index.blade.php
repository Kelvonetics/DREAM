...
<section class="tables-data">
	...
	<h1>      <i class="fa fa-map-marker"></i>     Company Location </h1> 
            <p class="lead"> The company location module  allows you to efficiently manage your entire company location. </p>  <br>
	 <a href="{{route('companylocation.add')}}" class='btn btn-primary' style='font-size:11px;color:white' title='Create New Company Location For Users'>New Location</a>
    ...
    <table id="example" class="table table-full table-full-small location" cellspacing="0" width="100%">
        <thead>
            <tr>
		...
		@if($companylocations)
            ...
            <tr>
				<td>{{$companylocations->LocationName}}</td>
				<td>{{$companylocations->Address}}</td>
				<td>{{$companylocations->State}}</td>
				<td>
                	<?php		
						$id = $companylocations->CountryId;
						$country = DB::table('country')->where('CountryId', '=', $id)->first();
					?> @if($country) {{$country->CountryName}} @endif
				</td>
				<td class="actions pull-right">
				
                <a href="{{route('companylocation.edit', $companylocations->LocationId)}}" class='btn btn-info fa fa-edit' style='color:white; font-size:9px'></a>								
