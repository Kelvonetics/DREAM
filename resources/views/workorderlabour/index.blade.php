...
<section class="tables-data">
    ...
    <p class="lead"> The work order labour module  allows you to efficiently manage your entire work order labour. </p> <br>
	 <a href="{{route('workorderlabour.add')}}" class='btn btn-primary' style='font-size:11px;color:white' title='Create New Work Order Labour For Part'>New Labour</a>
...
<div class="col-md-3 col-md-push-9 left-side" style="margin:-50px -20px 0px 10px; background-color:#F9F9F9">
	  ...
	  <div>
        ...
        <table id="example" class="table table-full table-full-small labour" cellspacing="0" width="100%">
        <thead>
            <tr>
                <th> Labour Type </th>
                <th> Description </th>
		...
		@if($workorderlabours)
            ...
            <tr>
                <td>{{$workorderlabours->LabourType}}</td>
                <td>{{$workorderlabours->LabourDesc}}</td>
                <td>{{$workorderlabours->LabourCost}}</td>

                <td class="actions pull-right">					
                <a href="{{route('workorderlabour.edit', $workorderlabours->LabourId)}}" class='btn btn-info fa fa-edit' style='color:white; font-size:9px'></a>	 
                </td>
            </tr>
            @endforeach
		@endif
