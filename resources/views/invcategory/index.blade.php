...
<section class="tables-data">
    ...
    <p class="lead"> The inventory category module  allows you to efficiently manage your entire inventory category. </p> <br>
	 <a href="{{route('invcategory.add')}}" class='btn btn-primary' style='font-size:11px;color:white' title='Create New Category For Part'>New Category</a>
...
<div class="col-md-3 col-md-push-9 left-side" style="margin:-50px -20px 0px 10px; background-color:#F9F9F9">
	  ...
	  <div>
        ...
        <table id="example" class="table table-full table-full-small invcat" cellspacing="0" width="100%">
        <thead>
            <tr>
                <th> Category Id </th>
                <th> Category </th>
		...
		@if($invcategorys) 
            ...
            <tr>
                <td>{{$invcategorys->InvCatId}}</td>
                <td>{{$invcategorys->InvName}}</td>

                <td class="actions pull-right">					
                <a href="{{route('invcategory.edit', $invcategorys->InvCatId)}}" class='btn btn-info fa fa-edit' style='color:white; font-size:9px'></a>	 
                </td>
            </tr>
            @endforeach
		@endif
