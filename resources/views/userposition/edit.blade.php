...
<div class="main-content row" autoscroll="true" bs-affix-target="" init-ripples bg="" style="margin-top:-100px">
  <section class="tables-data">
	...
	<h1>      <i class="fa fa-street-view"></i>     User Position </h1> 
	<p class="lead"> The user position module  allows you to efficiently manage your entire user position. </p>
	</div>
	...
	<div class="">
      ...
      <form class="form-horizontal" method="post" action="{{ url('/userposition/update', array($userposition->PositionId)) }}">
		<LEGEND> New User Position </LEGEND>
			...
			<tr class="box-section">
				<td width="50%">
					<div class="form-group">
						<label for="PositionName" class="control-label label-left"><i class="fa fa-street-view"></i> Position Name</label>
						<input type="text" rows="2" name="PositionName" id="PositionName" placeholder="User Position" style="border:thin #ede solid;	width:93%;	padding:5px;	border-radius:2px;margin:auto 2% auto 4%; color:#999"
						@if($userposition) value="{{$userposition->PositionName}}"@endif Required onblur="convert_name()"> 
					</div>
					<input type="hidden" name="CreatedBy" id="CreatedBy" class="form-control" value="{{$userposition->CreatedBy}}" readonly > 
                    <input type="hidden" name="_token" value="{{{ csrf_token() }}}">								
			  </td>
				<td width="50%"> 
					<div class="form-group">
						<label for="PositionDesc" class="control-label label-left"><i class="fa fa-pencil"></i> Position Description</label>
						<textarea type="text" name="PositionDesc" id="PositionDesc" style="border:thin #ede solid;	width:93%;	padding:5px;	border-radius:2px;margin:auto 0% auto 3%; color:#999" onblur="convert_desc()" required>@if($userposition){{$userposition->PositionDesc}}@endif</textarea>
					</div> 
				</td>
...
<script>
	...
	{
		var str = document.getElementById('PositionDesc').value;
		var splitStr = str.toLowerCase().split(' ');
	   for (var i = 0; i < splitStr.length; i++) {
		   // You do not need to check if i is larger than splitStr length, as your for does that for you
		   // Assign it back to the array
	   ...
	   // Directly return the joined string
	   var cap = splitStr.join(' '); 
			document.getElementById('PositionDesc').value = cap;
	}
</script>
