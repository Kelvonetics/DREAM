...
<div class="main-content row" autoscroll="true" bs-affix-target="" init-ripples bg="" style="margin-top:-100px">
  <section class="tables-data">
	...
	<h1>      <i class="fa fa-sitemap"></i>     User Department </h1> 
	<p class="lead"> The user department module  allows you to efficiently manage your entire user department. </p> 
	</div>
	...
	<div class="">
      ...
      <form class="form-horizontal" method="post" action="{{ url('/department/update', array($department->DeptId)) }}">
			...
			<tr class="box-section">
				<td width="50%">
					<div class="form-group">
						<label for="DeptName" class="control-label label-left"><i class="fa fa-sitemap"></i> Department Name</label>
						<input type="text" rows="2" name="DeptName" id="DeptName"  placeholder="Department Name" style="border:thin #ede solid;	width:93%;	padding:5px;	border-radius:2px;margin:auto 2% auto 4%; color:#999"@if($department) value="{{$department->DeptName}}"@endif Required onblur="convert_name()"> 
					</div>
					<input type="hidden" name="CreatedBy" id="CreatedBy" class="form-control" value="{{$department->CreatedBy}}" readonly > 
...
<script>
	function convert_name() 
	{
	   ...
	   var cap = splitStr.join(' '); 
			document.getElementById('DeptName').value = cap;
	}
</script>		
