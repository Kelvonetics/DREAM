...
<section class="tables-data">
	...
	<h1>      <i class="fa fa-car"></i>     Vehicle Model </h1> 
	<p class="lead"> The vehicle model module  allows you to efficiently manage your entire vehicle model. </p>
	</div>
	...
	<div class="">
      ...
      <form class="form-horizontal" method="post" action="{{ url('/assetmodel/update', array($assetmodel->ModelId)) }}">
      <LEGEND> Edit Vehicle Model </LEGEND>
      @if(count($errors) > 0)
		...
		<CAPTION>Model Details</CAPTION>
			<tr class="box-section">
				<td width="50%">
                    ...
                    <div class="form-group">
						<label for="MakeId" class="control-label label-left"><i class="fa fa-car"></i> Asset Make</label>
						<select class='sel-opt-left' name='MakeId' id='MakeId' required>
							<?php								
								$id = $assetmodel->MakeId;
								$assetmake = DB::table('assetmake')->where('MakeId', '=', $id)->first();
							?>
							@if($assetmake) <option value="{{{ $assetmake->MakeId }}}"> {{ $assetmake->Make }} </option>@endif
				...
				<td width="50%"> 
                    <div class="form-group">
                        <label for="ModelName" class="control-label label-right"><i class="fa fa-car"></i> Model Name</label>
                        <input type="text" name="ModelName" id="ModelName" placeholder="Asset Model Name" style="border:thin #ede solid;	width:93%;	padding:2px 5px;	border-radius:2px;margin:auto 0% auto 3%; color:#999" @if($assetmodel) value="{{$assetmodel->ModelName}}" @endif Required onblur="convert_model()"> 
                    </div>
                        <input type="hidden" name="CreatedBy" id="CreatedBy" class="form-control"@if($assetmodel) value="{{$assetmodel->CreatedBy}}"@endif readonly >
			...
			<td colspan="2">
				<div class="form-group" style="margin-left:-6px">
					<button type="submit" class="btn btn-primary">Update Vehicle Model</button>
...
<script>
	...
	{
		var str = document.getElementById('ModelName').value;
	   ...
	   var cap = splitStr.join(' '); 
			document.getElementById('ModelName').value = cap;
	}
</script>
