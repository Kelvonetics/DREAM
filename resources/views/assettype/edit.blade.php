...
<section class="tables-data">
	...
	<h1>      <i class="fa fa-car"></i>     Vehicle Type </h1> 
	<p class="lead"> The vehicle type module  allows you to efficiently manage your entire vehicle type. </p>
	</div>
	...
	<div class="">
      ...
      <form class="form-horizontal" method="post" action="{{ url('/assettype/update', array($assettype->AssetTypeId)) }}">
      ...
      @if(count($errors) > 0)
		...
		<CAPTION>Type Details</CAPTION>
			<tr class="box-section">
				<td width="50%">

                <div class="form-group">
                    <label for="AssetTypeName" class="control-label label-left"><i class="fa fa-car"></i> Type Name</label>
                    <input type="text" name="AssetTypeName" id="AssetTypeName" placeholder="Asset Type Name" style="border:thin #ede solid;	width:93%;	padding:5px;	border-radius:2px;margin:auto 2% auto 4%; color:#999"@if($assettype) value="{{ $assettype->AssetTypeName }}"@endif Required onblur="convert_type()"> 
                </div>
                    <input type="hidden" name="CreatedBy" id="CreatedBy" class="form-control"@if($assettype) value="{{ $assettype->CreatedBy }}"@endif readonly >
...
<script>
	...
	{
		var str = document.getElementById('AssetTpyeName').value;
	   ...
	   var cap = splitStr.join(' '); 
			document.getElementById('AssetTpyeName').value = cap;
	}
</script>
