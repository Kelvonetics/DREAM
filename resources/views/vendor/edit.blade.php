...
<section class="tables-data">
	...
	<p class="lead"> The vendor module  allows you to efficiently manage your entire vendor. </p>
	</div>
	...
	<div class="">
      ...
      <form class="form-horizontal" method="post" action="{{ url('/vendor/update', array($vendor->VendorId)) }}">
      <LEGEND> Edit Part Vendor </LEGEND>
        @if(count($errors) > 0)
			...
			<tr class="box-section">
				<td width="50%">
                    ...
                    <div class="form-group">
						<label for="InvCatId" class="control-label label-left"><i class="fa fa-list"></i> Vendor Category</label>
						<select class='sel-opt-left' name='InvCatId' id='InvCatId' required>
							<?php            
								$invid = $vendor->InvCatId;		
								$invcategory = DB::table('invcategory')->where('InvCatId', '=', $invid)->first();
							 ?>
							 <option value="{{{ $invcategory->InvCatId }}}"> {{ $invcategory->InvName }} </option>
							 @if($invcategory)
							 <option value="" style="color:#202020">Select Part Category</option>
							 @endif
				...
				<td width="50%"> 
                    <div class="form-group">
                        <label for="VendorName" class="control-label label-right"><i class="fa fa-car"></i> Vendor Name</label>
                        <input type="text" name="VendorName" id="VendorName" placeholder="Vendor Name" style="border:thin #ede solid;	width:93%;	padding:2px 5px;	border-radius:2px;margin:auto 0% auto 3%; color:#999" @if($vendor) value="{{$vendor->VendorName}}"@endif Required onblur="convert_vendor()"> 
                    </div>
                        <input type="hidden" name="CreatedBy" id="CreatedBy" class="form-control" value="{{$vendor->CreatedBy}}" readonly >
...
<script>
	...
	{
		var str = document.getElementById('VendorName').value;
	   ...
	   var cap = splitStr.join(' '); 
			document.getElementById('VendorName').value = cap;
	}
</script>
