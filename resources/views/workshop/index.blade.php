...
<div class="main-content row" autoscroll="true" bs-affix-target="" init-ripples bg="" style="margin-top:-100px">
  <section class="tables-data">
	...
	<h1>      <i class="fa fa-truck"></i>     Workshop </h1> 
    <p class="lead"> The workshop module  allows you to efficiently manage your entire workshop. </p> <br>
	 <a href="{{route('workshop.add')}}" class='btn btn-primary' style='font-size:11px;color:white' title='Create New Auto Workshop For Part/Asset'>New Workshop</a>
	...
	<div class="">
        ...
        <table id="example" class="table table-full table-full-small shop" cellspacing="0" width="100%">
        <thead>
            <tr>
                <th> Workshop </th>
                <th> Phone </th>
		...
		@if($workshops)
            ...
            <tr>
                ...
                <td>{{$workshops->Phone}}</td>
                <td>{{$workshops->Address}}</td>
                <td>{{$workshops->City}}</td>
                <td>{{$workshops->OperationHoursFrom}}</td>
                <td>{{$workshops->OperationHoursTo}}</td>

                <td class="actions pull-right">					
                <a href="{{route('workshop.edit', $workshops->WorkShopId)}}" class='btn btn-info fa fa-edit' style='color:white; font-size:9px'></a>	 
                </td>
            </tr>
            @endforeach
		@endif
