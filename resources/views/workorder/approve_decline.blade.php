...
<br> or work activity. This module offers a single familiar tool for scheduling, managing and collecting data for all work activities. </p>
   ...
   <!-- left content -->
	  ...
	  <div>
      <form class="form-horizontal" method="post" action="{{ url('/workorder/approve', array($workorder->WOId)) }}">
		<fieldset>
		  ...
		  <div class="lead" style="margin:0px 0px 40px -15px">
				...
				@if($date)
					<?=  $date->ServiceDate.' &nbsp; - &nbsp; '.$date->ServiceCompletionDate;   ?> 
				@endif  
			    </div>
		   </div>
		  
		  <div class="form-group"> 
            <input type="hidden" class="form-control" name="WorkOrderNumber" id="WorkOrderNumber" value="{{$workorder->WorkOrderNumber}}" Required Readonly>
			<input type="hidden" class="form-control" name="WOId" id="WOId"
			@if($workorder) value="{{$workorder->WOId}}"@endif Required Readonly>
		 </div> 
		...
		<CAPTION>Work Order Details</CAPTION>
			<tr class="box-section">
				...
				<label for="ActualStartDate" class="control-label label-left"><i class="fa fa-calendar"></i> Actual Start Date</label>
					...
					@if($date) value="<?= $date->ActualStartDate ?>"@endif maxlenght="10" Readonly /> 	
				</div>
				...
				<div class="form-group">
				  <label for="MaintenanceType" class="control-label label-left"><i class="fa fa-list"></i> Maintenance Type</label>
					 <select class='sel-opt-left' name='MaintenanceType' id='MaintenanceType' required>				@if($date)
				...
				<td width="50%"> 
					...
					<div class="form-group">
						...
						<input type='text' id='ActualEndDate' name='ActualEndDate' class="datepicker" placeholder="MM/DD/YYYY" style="border:thin #ede solid;	width:93%;	padding:5px;	border-radius:2px;margin:auto 0% auto 3%; color:#999"
							@if($date) value="<?= $date->ActualEndDate  ?>"@endif Readonly /> 	
					</div>
					...
					<div class="form-group">
						...
						@if($date)	value="<?= $date->ServiceCompletionDate  ?>"@endif readonly required/> 	
					</div>
								
					<div class="form-group">	  
					  <div class="controls">
					  <label for="WorkOrderStatus" class="control-label label-right"><i class="fa fa-book"></i> Work Order Status</label>  
						 <select class='sel-opt-right' name='WorkOrderStatusId' id='WorkOrderStatusId' required Readonly>						   
							<?php 
                            ...
                            ?>
							@if($sta)
							<option class='option' value="{{$sta->WorkOrderStatusId}}">
							{{$sta->WorkOrderStatus}}
							</option>
							@endif
						</select>
			...
			<tr class="box-section">
				...
				<td> 
					<div class="form-group">
						...
						<input type="number" name="OdometerReading" id="OdometerReading" style="border:thin #ede solid;	width:93%;	padding:5px;	border-radius:2px;margin:auto 0% auto 3%; color:#999" @if($date)
						value="{{$date->OdometerReading}}"@endif required Readonly> 
					</div>
				</td>
			</tr>
			...
			<tr>
				...
				if($fleet){ echo $fleet->FleetManagerEmail; }
				?>">
				</td> 
				<td> 
					<input type="hidden" name="ShopEmailId" id="ShopEmailId" value="@if($fleet){{$fleet->ShopEmailId}} @endif" Readonly> <!-- fleet mgt -->
					<input type="hidden" name="ShopMechId" id="ShopMechId" value="@if($mechanic){{$mechanic->ShopEmailId}} @endif" Readonly> <!-- vendor -->
                    <input type="hidden" name="shopemail" id="shopemail" value="@if($mechanic){{$mechanic->Email}} @endif" Readonly> <!-- vendor -->
					<input type="hidden" name="RoleId" id="RoleId" value="{{$auth_user->RoleId}}" Readonly> <!-- login user roleId -->
				</td>
			</tr>
		</table>  
		...
		<?php	
			...
			<tr id="row".$i."">  <td width="85%"  class="box-section">
				<div class="form-group col-md-4">	  
					...
					<input type="hidden" name="WOId".$i."" id="WOId".$i.""@if($records) value="{{$records->WOId}}"@endif Readonly />
					<input type="hidden" name="WorkOrderNumber".$i."" id="WorkOrderNumber".$i.""
					@if($records) value="{{$records->WorkOrderNumber}}"@endif Readonly />
					</div>  </div>     <div class="form-group col-md-8">   <div class="controls">
							
		
					<label for="InvId".$i."" class="fm-label label-right"><i class="fa fa-list"></i> Item Name </label>  <?php $pid = $records->InvId; ?>
				...
				<td width="13%" style="vertical-align:top; text-align:left; border: none;"> 
					<div class="form-group">
						...
						<input type="number" name="PartCost".$i."" id="PartCost".$i."" class="pcost" onkeyup="checkAmount(this)" style="border:thin #ede solid;	width:93%;	padding:5px;	border-radius:2px;margin:auto 2px auto 10px; color:#999"@if($records) value="{{$records->PartCost}}"@endif Readonly /> 										   
					</div>
                ...
                <button type="button" name="" id="" class="btn btn-circle-red btn_remove"  disabled>X</button>
		...
		<input type="hidden" name="count_part" id="count_part"@if($count_part) value="{{$count_part}}"@endif class="form-control" Readonly>
		</td></tr>
		</table>
		...
		<?php	
			...
			<tr id="rows".$c.""  class="">
				...
				<div class="form-group col-md-12">	
					...
					<input type="hidden" name="WOId_l".$c."" id="WOId_l".$c.""@if($records) value="{{$records->WOId}}"@endif Readonly />
					<input type="hidden" name="WorkOrderNumber".$c."" id="WorkOrderNumber".$c.""
					@if($date) value="{{$records->WorkOrderNumber}}"@endif Readonly />
					</div>
						<?php  $no = $c + 1;  ?>
				...
				<td width="13%" style="vertical-align:top; text-align:left;"> 
					<div class="form-group">
						...
						@if($records) value="{{$records->LabourCost}}"@endif Readonly /> 											   
					</div>  
		...
		<table class="table" cellspacing="1" width="100%" border="0" style="">
			<tr><td>  
			<input type="hidden" name="count_lab" id="count_lab" value="<?= $count_lab ?>" class="form-control" Readonly>
			</td></tr>
		</table>
		...
		<tr style="background-color:#f9f9f9; width:104%">
			...
			<td width="20%" style=" text-align:left;"> 
				<div class="form-group">
					...
					<input type="hidden" name="TotalCost" id="TotalCost" Readonly>
				</div>
			</td>
			...
			<tr id=""  class="">
				<td width="100%"  class="box-section labour-box" colspan="2"> 
					<div class="form-group labour-box">	  
						...
						?> @if($shop) 
							<option class='option' value="{{$shop->WorkShopId}}">{{$shop->WorkShopName}}</option>
						@endif
						</select>
					</div>
			...
			<tr id=""  class="">
				<td width="100%"  class="box-section labour-box" colspan="2"> 
					<div class="form-group labour-box">	  
						...
						<textarea rows="3" name="Comment" id="Comment" onkeyup="convert_comm()" placeholder="Comment" style="border:thin #ede solid;	width:100%;	padding:5px;	border-radius:2px;margin:auto 0% auto 0%; color:#999" Readonly>@if($date){{$date->Comment}}@endif</textarea>					
                    </div>				
				</td>
			  ...
			  ?>
					<div class="form-group" style="margin-left:10px">
						<button type="submit" class="btn btn-success" style="background-color:#396">Approve</button>
...
<!-- Order Decline Modal -->						
<form class="form-horizontal" method="POST" action="{{ url('/workorder/decline', array($workorder->WOId) )}}">	
<div id="declineModal" class="modal fade" role="dialog" style="height:60%; margin:2% auto;">
  <div class="modal-dialog" style="width:50%;">
    ...
    <div class="modal-content">
      <div class="modal-header">
        ...
        <h4 style="color:red;">  <i class="fa fa-wrench" aria-hidden="true"></i> Decline Work Order </h4>
      </div>
      <div class="modal-body">
       

	  @include('modals.decline_order')
