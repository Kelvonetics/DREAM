...
<section class="tables-data">
	...
	<h1>      <i class="fa fa-car"></i>     Asset Model </h1> 
            <p class="lead"> The Asset Model module  allows you to efficiently manage your entire asset Model. </p>  <br>
	 <a href="{{route('assetmodel.add')}}" class='btn btn-primary' style='font-size:11px;color:white' title='Create New Model For Asset'>New Model</a>
...
<div class="col-md-3 col-md-push-9 left-side" style="margin:-50px -20px 0px 10px; background-color:#F9F9F9">
	  ...
	  <div>
        ...
        <table id="example" class="table table-full table-full-small model" cellspacing="0" width="100%">
        <thead>
            <tr>
                <th> ModelId </th>
                <th> Make Name </th>
		...
		@if($assetmodels)
            ...
            <tr>
                <td>{{$assetmodels->ModelId}}</td>
                <td>
				<?php 
						$mkid = $assetmodels->MakeId;		
						$assetmake = DB::table('assetmake')->where('MakeId', '=', $mkid)->first();	
					?> @if($assetmake) {{$assetmake->Make}} @endif
				</td>
                <td>{{$assetmodels->ModelName}}</td>

                <td class="actions pull-right">					
                <a href="{{route('assetmodel.edit', $assetmodels->ModelId)}}" class='btn btn-info fa fa-edit' style='color:white; font-size:9px'></a>	 
                </td>
            </tr>
            @endforeach
		@endif
